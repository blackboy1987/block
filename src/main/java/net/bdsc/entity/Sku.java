...
import net.bdsc.BaseAttributeConverter;
import net.bdsc.Setting;
import net.bdsc.plugin.GiftPromotionPlugin.GiftAttribute;
...
public class Sku extends BaseEntity<Long> {
	 ...
	 */
	@Transient
	public net.bdsc.entity.Store getStore() {
	 ...
	 */
	@Transient
	public BigDecimal getPlatformCommission(net.bdsc.entity.Store.Type type) {
		...
		if (type != null && getProduct() != null && getProduct().getProductCategory() != null) {
			...
			if (net.bdsc.entity.Store.Type.GENERAL.equals(type) && productCategory.getGeneralRate() > 0) {
				commission = getPrice().multiply(new BigDecimal(String.valueOf(productCategory.getGeneralRate())).movePointLeft(2));
			} else if (net.bdsc.entity.Store.Type.SELF.equals(type) && productCategory.getSelfRate() > 0) {
